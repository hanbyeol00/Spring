package com.han.contect.service.impl;
...
public class ContactServiceImplV1 implements ContactService {
	 ...
	 * 무언가 싱행을 하려고 하면 Null Pointer Exception이
	 * 주로 발생한다
	 * 
	 * Null Pointer Exception 문제는 오래전부터 Java 개발자를
	 * 매우 괴롭혔다
	 */
	// private Random rnd = new Random(); 이렇게 하지말고
	// 이렇게 선언만 하고 아래 생성자에서 초기화
	private Random rnd;
	// private String[] names = new String[] { "홍길동", "이몽룡", "성춘향", "임꺽정", "장보고" };
	private String[] names;

	public ContactServiceImplV1() {
		// 기본 생성자에서 names 인스턴스 배열을
		// 5개의 이름이 담긴 문자열 배열로 만들기
		this.names = new String[] { "홍길동", "이몽룡", "성춘향", "임꺽정", "장보고" };
	...
	public Contact finById(String id) {
		...
		/**
		 * Contact 클래스를 사용하여 Contact 정보를 저장할 myContact 객체선언 생성자(new Contact()) 를 사용하여
		 * myContact 객체를 생성 => 인스턴트 화
		 * 
		 * 이때 Contact 에 선언된 변수들은 모두 초기화가 된다
		 */
		Contact myContact = new Contact("S0001", "홍길동", "gksquf6670@gmail.com", 33);
