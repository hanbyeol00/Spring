...
public class HomeController {
	...
	/**
	 * 일반 컨트롤러에서 String 데이터를 return 하는 경우 /WEB-INF/views/ 폴더에 저장된 JSP 파일을 rendering
	 * 하여 response 하라
	 */
	...
	/**
	 * method 에 ResponseBody 가 설정되고 객체를 return 하는 경우 데이터를 JSON type 으로 response 하라
	 */
	@ResponseBody
	...
	public BookDto getMyBook() {
		 ...
		 * 클래스를 사용하여 객체를 생성하는데, 다소 복잡한 규칙이 적용된다
		 * 
		 ...
		 * 2. 객체를 생성할때 클래스.builder() method 먼저 호출하여
		 * 3. 객체를 초기화하고,
		 * 4. 각 맴버변수 이름을 직접사용하여
		 * 5. 값을 setter 하고
		 * 6. build() method 를 호출하여 객체를 완전 생성하는 구조
		 * 
		 * 필요한 맴버변수 값만 초기화 할수 있고,
		...
		// 사용하려면 그에 맞는 생성자 metgod 가 준비되어야 한다
